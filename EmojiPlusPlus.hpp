...
#ifndef EMOJI_PLUS_PLUS_HPP
#define EMOJI_PLUS_PLUS_HPP
...
namespace emojiplusplus {
    ...
    // From github.com/ferhatgec/stringtools
    #if USE_SPLITTER
    ...
    #if USE_SPLITTER
        static std::vector<std::string> SplitEmojiString(std::string s) {
            std::istringstream stream(s);
            std::vector<std::string> data;

            for(std::string temporary; stream >> temporary;) {
    ...
    #if USE_SPLITTER
        // Tokenizer based parser, replacer.
        // Hi :...: huh :...: !
        //   ^     ^   ^     ^
        // Whitespaces required.
        static std::string EmojiToken(std::string s) {
            std::string copy_data(s);

            std::vector<std::string> data = SplitEmojiString(copy_data);

            for(auto& token : data) {
                if(token.front() == ':') {
                    auto iterator = emojiplusplus::Emojis.find(token);

                    if(iterator != emojiplusplus::Emojis.end()) {
                        ReplaceAll(copy_data, token, iterator->second);
                    }
                }
            }
